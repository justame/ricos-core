...
import { removeColSpan, TableMap, CellSelection, tableNodeTypes } from 'prosemirror-tables';
    ...
    for (let row = 0, i = 0; row < newHeight; row++, i++) {
      ...
      for (let j = 0; j < source.childCount; j++) {
        ...
        if (row + cell.attrs.rowspan > newHeight)
          cell = cell.type.create(
            { ...cell.attrs, rowspan: Math.max(1, newHeight - cell.attrs.rowspan) },
            cell.content
          );
        cells.push(cell);
      }
...
function growTable(tr, map, table, start, width, height, mapFrom) {
  ...
  if (height > map.height) {
    ...
    for (let i = 0, start = (map.height - 1) * map.width; i < Math.max(map.width, width); i++) {
      const header =
        i >= map.width ? false : table.nodeAt(map.map[start + i]).type === types.header_cell;
      cells.push(
        header
  ...
  for (let col = left; col < right; col++) {
    ...
    if (map.map[index - map.width] === pos) {
      ...
      tr.insert(
        ...
        cell.type.createAndFill({ ...cell.attrs, rowspan: cellTop + cell.attrs.rowspan - top })
      );
